...
class CarlaEnv(gym.Env):
    def __init__(self, params):
        ...
        self.vehicle_spawn_points = [sp for sp in self.map.get_spawn_points()
                       if distance(sp.location, carla.Location(x=-45.248, y=20.426, z=0)) < 30.0]
        ...
        self.start_waypoint1 = self.map.get_waypoint(carla.Location(x=-66.826, y=24.498, z=0),project_to_road=True,lane_type=carla.LaneType.Driving)
        self.start_spawn_point1 = [sp for sp in self.map.get_spawn_points() if distance(sp.location, carla.Location(x=-64.644, y=24.471, z=0.1)) < 1][0]
        self.start_waypoint2 = self.map.get_waypoint(carla.Location(x=-45.186, y=42.545, z=0),project_to_road=True,lane_type=carla.LaneType.Driving)
        self.start_spawn_point2 = [sp for sp in self.map.get_spawn_points() if distance(sp.location, carla.Location(x=-48.819, y=-4.795, z=0.1)) < 1][0]

        self.end_point = None

        self.end_point_left1_1 = self.map.get_waypoint(carla.Location(x=-41.811, y=-1.704, z=0),project_to_road=True,lane_type=carla.LaneType.Driving)
        self.end_point_left1_2 = self.map.get_waypoint(carla.Location(x=-45.311, y=-1.705, z=0), project_to_road=True,lane_type=carla.LaneType.Driving)
        #self.end_point_forward1 = self.map.get_waypoint(carla.Location(x=-28.716, y=24.004, z=0), project_to_road=True,lane_type=carla.LaneType.Driving)

        self.end_point_left2_1 = self.map.get_waypoint(carla.Location(x=-25.516, y=24.613, z=0), project_to_road=True,lane_type=carla.LaneType.Driving)
        self.end_point_left2_2 = self.map.get_waypoint(carla.Location(x=-28.726, y=28.104, z=0), project_to_road=True,lane_type=carla.LaneType.Driving)
        #self.end_point_forward2 = self.map.get_waypoint(carla.Location(x=-48.673, y=46.955, z=0), project_to_road=True,lane_type=carla.LaneType.Driving)


        # 使用Carla的全局路径规划器
        self.dao = GlobalRoutePlanner(self.map, sampling_resolution=2.0)
    def reset(self, start_point=None, end_point=None):
        ...
        while True:
            ...
            available_spawn_points = [
                sp for sp in [self.start_spawn_point1, self.start_spawn_point2] if sp not in self.used_spawn_points
            ]

            if len(available_spawn_points) > 0:
                start_transform = random.choice(available_spawn_points)  # Choose a spawn point not used by nearby vehicles
                if self._try_spawn_ego_vehicle_at(start_transform):
                    #print(f"ego transform: {start_transform}")
                    self.start_point = self.map.get_waypoint(start_transform.location, project_to_road=True,
                                                             lane_type=carla.LaneType.Driving)
                    # print(f"ego vehicle spawned at: {self.ego.get_location()}")
                    #print(f"start point: {self.start_point.transform.location}")
            ...
            #    self.reset()
                ...
                #time.sleep(0.1)
                #self.reset()
            #print(f"number of available spawn points: {len(available_spawn_points)}")
            #print(f"number of spawn points: {len(self.vehicle_spawn_points)}")
            # Try to spawn the ego vehicle at the selected location

            # if self._try_spawn_ego_vehicle_at(start_transform):
            #     print(f"ego transform: {start_transform}")
            #     self.start_point = self.map.get_waypoint(start_transform.location, project_to_road=True, lane_type=carla.LaneType.Driving)
            #     #print(f"ego vehicle spawned at: {self.ego.get_location()}")
            #     print(f"start point: {self.start_point.transform.location}")
            #     break  # Successfully spawned the ego vehicle
        ...
        #             size=0.5,
        #             life_time=5,
        #             color=carla.Color(0, 255, 0) ) #spawn point: green
        #
        # self.world.debug.draw_point(
        #             self.start_point.transform.location,
        #             size=0.5,
        #             life_time=5,
        #             color=carla.Color(0, 0, 255) ) #start point: blue
            

        #available_end_points = [p for p in self.vehicle_spawn_points if p != self.ego.get_transform()]
        if start_transform == self.start_spawn_point1:
            self.end_point = random.choice([self.end_point_left1_1,self.end_point_left1_2])
        if start_transform == self.start_spawn_point2:
            self.end_point = random.choice([self.end_point_left2_1,self.end_point_left2_2])

        self.end_point = self.map.get_waypoint(self.end_point.transform.location, project_to_road=True, lane_type=carla.LaneType.Driving)
        # self.world.debug.draw_point(
        #             self.end_point.transform.location,
        #             size=0.5,
        #             life_time=50,
        #             color=carla.Color(0, 0, 0) ) #end point: black

        # 生成从起点到终点的路径
        self.route = self.dao.trace_route(
            self.start_point.transform.location,
            self.end_point.transform.location
        )
        self.waypoints = [waypoint[0] for waypoint in self.route]

        # for waypoint, road_option in self.route:
        #     self.world.debug.draw_point(
        #         waypoint.transform.location + carla.Location(z=0.5),
        #         size=0.1,
        #         color=carla.Color(r=255, g=0, b=0), # waypoint: red
        #         life_time=5.0
        #     )
        #
        #     # 可选：画箭头表示朝向
        #     forward_vector = waypoint.transform.get_forward_vector() * 1.0
        #     self.world.debug.draw_arrow(
        #         waypoint.transform.location + carla.Location(z=0.3),
        #         waypoint.transform.location + forward_vector + carla.Location(z=0.3),
        #         thickness=0.05,
        #         arrow_size=0.1,
...
# ========================== Current reference waypoints (up to N waypoints) ==========================
        ...
        #waypoint = self.map.get_waypoint(self.ego.get_location())  # TODO: ROUTE PLANNING USING WORLD_MAP.GET_WAYPOINT
        waypoint = self._find_nearest_waypoint_ahead(self.ego, self.waypoints, max_waypoints)
        waypoints_array = np.zeros((max_waypoints, 3), dtype=np.float32)

        #print(f"all of waypoint is {self.waypoints}, length is {len(self.waypoints)}")
        #print(f"current waypoint is {waypoint}, all of the route is {self.route}, length is {len(self.route)}")
        ...
        for i in range(max_waypoints):
            ...
            # Move to the next waypoint, 超出索引的部分默认为最后一个点
            if (self.current_waypoint_index + i) >= len(self.waypoints):  
                next_waypoints = self.waypoints[-1]
            else:
                next_waypoints = self.waypoints[self.current_waypoint_index + i]
            waypoint = next_waypoints 
        
        obs['waypoints'] = waypoints_array.flatten()

# ============================= Lane boundary information =========================================
        waypoint_center = self.map.get_waypoint(
...
# =============================== Visualize current reference waypoints ===============================
        if self.visualize_waypoints:
            for i in range(max_waypoints):
                ...
                self.world.debug.draw_point(
                    ...
                    size=0.1,
                    life_time=0.5,
                    color=carla.Color(56, 123, 200)  # current waypoints 
                )
    ...
    def _terminal(self):
        ...
        if self.end_point is not None:
            end_point_x = self.end_point.transform.location.x
            end_point_y = self.end_point.transform.location.y
            if np.sqrt((ego_x - end_point_x)**2 + (ego_y - end_point_y)**2) < 4:
                return True
    ...
    def _find_nearest_waypoint_ahead(self, vehicle, waypoints, max_waypoints):
        """
        查找车辆前方指定距离内最近的waypoint
        
        Args:
            vehicle: carla.Vehicle - 车辆对象
            waypoints: list - waypoint列表
            look_ahead_distance: float - 前瞻距离
        ...
        # 车辆前进方向的单位向量
        forward_vector = np.array([math.cos(vehicle_yaw), math.sin(vehicle_yaw)])
        nearest_waypoint = None
        min_distance = float('inf')

        #从当前waypoint开始遍历，往前取max_waypoints个点
        for i, waypoint in enumerate(waypoints[self.current_waypoint_index:],start=self.current_waypoint_index):

            start_index = self.current_waypoint_index
            # 获取waypoint位置
            if hasattr(waypoint, 'transform'):
                wp_pos = np.array([waypoint.transform.location.x, waypoint.transform.location.y])
            elif hasattr(waypoint, 'location'):
                wp_pos = np.array([waypoint.location.x, waypoint.location.y])
            elif isinstance(waypoint, (list, tuple)) and len(waypoint) >= 2:
                wp_pos = np.array([waypoint[0], waypoint[1]])
            else:
                continue
            
            # 计算从车辆到waypoint的向量
            to_waypoint = wp_pos - vehicle_pos
            # 计算投影长度（判断waypoint是否在车辆前方）
            #projection = np.dot(to_waypoint, forward_vector)
            # 只考虑前方的waypoint
            ...
            #  超出指定数量的waypoint，结束遍历
            if (i-start_index) >= max_waypoints:
